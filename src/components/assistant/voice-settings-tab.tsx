...
import { Volume2, Settings, ListFilter, Palette, AlertCircle, Loader2, PlayCircle, X } from 'lucide-react';
...
const ACTUAL_PREVIEW_TEXT = "This is an actual audio preview generated by ElevenLabs. I hope you like the way I sound with the current settings!";

function SubmitActualPreviewButton({ isGenerating }: { isGenerating: boolean }) {
  return (
    <Button type="submit" variant="default" disabled={isGenerating} className="w-full md:w-auto">
...
export default function VoiceSettingsTab() {
  ...
  const handlePreviewGeneration = async (formData: FormData) => {
    ...
    try {
      ...
      if (!text || !voiceId) {
        throw new Error('Missing required fields: text or voiceId');
      }

      console.log('[VoiceSettingsTab] Sending request to voice preview API');
      ...
      const response = await fetch('/api/voice-preview', {
        ...
        body: JSON.stringify({
          text,
          voiceId,
          model_id: voiceConfig?.providerSpecific?.elevenlabs?.model_id || 'eleven_multilingual_v2',
          stability: voiceConfig?.providerSpecific?.elevenlabs?.stability ?? 0.5,
          similarity_boost: voiceConfig?.providerSpecific?.elevenlabs?.similarity_boost ?? 0.5,
          style: voiceConfig?.providerSpecific?.elevenlabs?.style ?? 0.0,
          use_speaker_boost: voiceConfig?.providerSpecific?.elevenlabs?.use_speaker_boost ?? true,
        }),
      });
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to generate audio preview. Please try again.",
        variant: "destructive",
      });
    } finally {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            ...
            <div className="flex items-center space-x-2 pt-6">
              <Controller
                ...
                control={control}
                render={({ field }) => <Switch id="voice.loopBackgroundSound" type="button" checked={field.value} onCheckedChange={field.onChange} />}
              />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
             ...
             <div className="flex items-center space-x-2 pt-2">
              <Controller
                ...
                control={control}
                render={({ field }) => <Switch id="voice.smartChunking" type="button" checked={field.value} onCheckedChange={field.onChange} />}
              />
